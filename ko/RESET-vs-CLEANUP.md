...
| **삭제 방법** | namespace 전체 삭제 | 각 Lab 스크립트의 --cleanup 옵션 |
| **실행 순서** | 순서 무관 (namespace 일괄 삭제) | **역순** (21 → 01) |
...
1. **모든 poc-* namespace 삭제** (한 번에)
   - `oc delete namespace poc-*`
   - namespace 안의 모든 리소스도 함께 삭제
...
- **느림** - 21개 스크립트를 순차 실행
- **불완전할 수 있음** - 스크립트에 --cleanup 구현 안 된 경우
- **복잡함** - 각 스크립트의 cleanup 로직 필요
...
**방법 1: reset 사용**
```bash
./make.sh reset                    # 모든 POC namespace 삭제 (5초)
./setup.sh                    # 환경 재설정 필요
...
**방법 3: 직접 삭제**
```bash
oc delete namespace poc-oadp  # 해당 namespace만 삭제 (3초)
...
**클러스터 레벨 리소스**
- NNCP, Template은 namespace 밖에 있음
- `reset`으로는 삭제 안 됨
- 필요시 수동 삭제:
  ```bash
  oc delete nncp --all
...
- 부분적으로 = 직접 namespace 삭제 또는 개별 Lab --cleanup
