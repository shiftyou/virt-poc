...
# 이 스크립트를 실행하면 openshift namespace에 poc Template이 생성됩니다.
#
# 사용법: ./01-template.sh [qcow2-파일-경로]
#   예시) ./01-template.sh
#   예시) ./01-template.sh /path/to/vm-images/rhel9-poc-golden.qcow2
# =============================================================================
...
preflight() {
    ...
    print_ok "설정 확인됨 (STORAGE_CLASS=${STORAGE_CLASS})"
...
# =============================================================================
# Step 1: DataVolume 생성 (로컬 업로드 또는 HTTP URL import)
# =============================================================================
step_datavolume() {
    print_step "1/4  DataVolume 생성 (poc-golden)"
    ...
    if [ "$phase" = "Succeeded" ]; then
        print_ok "DataVolume $DV_NAME 이미 존재합니다 (Succeeded) — 건너뜀"
        return
    elif [ -n "$phase" ]; then
    ...
    if [ -n "$local_qcow2" ]; then
        print_info "로컬 파일 발견: ${local_qcow2} — virtctl 업로드 사용"
...
EOF
        ...
        print_info "${local_qcow2} 파일을 virtctl image-upload로 업로드 중..."
...
# =============================================================================
# Step 2: DataSource 등록 및 PVC Bound 대기
# =============================================================================
step_datasource() {
...
EOF
    ...
    while true; do
        ...
        if [ "$pvc_phase" = "Bound" ]; then
            print_ok "PVC $DV_NAME Bound 확인됨 — Template 생성을 진행합니다."
            break
        fi
        ...
        if [ "$dv_phase" = "Failed" ]; then
            ...
            print_error "DataVolume $DV_NAME 실패: ${dv_msg:-알 수 없는 오류}"
            print_error "Golden Image URL을 확인하거나 로컬 qcow2 파일을 제공하세요."
            exit 1
        fi
...
print_summary() {
    echo ""
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${GREEN}  완료! POC VM Template이 등록되었습니다.${NC}"
    echo -e "${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
