...
DASHBOARD_EOF
    ...
    print_ok "GrafanaDashboard poc-vm-statusmap-operator 배포됨 (벌집 모양 VM 배치도)"
    ...
    if [ -n "$grafana_route" ]; then
        ...
        print_info "  대시보드: https://${grafana_route}/d/poc-vm-statusmap-operator  (벌집 배치도)"
    fi
}
...
ensure_polystat_plugin() {
    ...
    if [ "$cp_ok" != "true" ]; then
        print_error "Grafana Pod에 연결할 수 없습니다."
        return 1
    fi
...
print_summary() {
    ...
    if [ -n "${GRAFANA_NS:-}" ]; then
        ...
        if [ -n "$grafana_route" ]; then
            ...
            echo -e "    - ${CYAN}https://${grafana_route}/d/poc-vm-statusmap-operator${NC}  (벌집 배치도)"
